							...
							<table class="table table-bordered table-hover table-sm mx-auto" style="text-overflow: ellipsis; width: 98%">
								...
								<tbody>
									...
									if (pageDTO.getTotalCount() > 0) {
										...
										%>
										<tr onclick="location.href='/board/boardView.jsp?num=<%=boardVO.getNum()%>&pageNum=<%=pageDTO.getCri().getPageNum()%>'">
											<td><%=boardVO.getNum()%></td>
											<td><%=boardVO.getSubject()%></td>
											<td><%=boardVO.getNick()%></td>
											<td><%=strRegDate%></td>
					...
					<nav aria-label="Page navigation">
						<ul class="pagination justify-content-center my-2">
							...
							<li class="page-item"><a class="page-link"
								href="/board/boardList.jsp?pageNum=<%=pageDTO.getStartPage() - 1%>&type=<%=pageDTO.getCri().getType()%>&keyword=<%=pageDTO.getCri().getKeyword()%>#board"
								aria-label="Previous"
							...
							if (pageDTO.getEndPage() == 0) {
								...
								<li class="page-item <%=(pageDTO.getCri().getPageNum() == i) ? "active" : ""%>"><a class="page-link"
									href="/board/boardList.jsp?pageNum=<%=i%>&type=<%=pageDTO.getCri().getType()%>&keyword=<%=pageDTO.getCri().getKeyword()%>#board"
								><%=i%></a></li>
								<%
								}
							}
							...
							%>
							<li class="page-item"><a class="page-link"
								href="/board/boardList.jsp?pageNum=<%=pageDTO.getEndPage() + 1%>&type=<%=pageDTO.getCri().getType()%>&keyword=<%=pageDTO.getCri().getKeyword()%>#board" aria-label="Next"
						...
						if (id != null) {
						%>
						<a href="/board/boardWrite.jsp" class="position-absolute" style="margin-top: -45px">
