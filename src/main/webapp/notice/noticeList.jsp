...
// notice 테이블에서 전체글 리스트로 가져오기 
List<NoticeVO> noticeList = noticeDAO.getNotices(cri);

// 전체 글개수 가져오기
int totalCount = noticeDAO.getCountBySearch(cri); // 검색유형, 검색어가 있으면 적용하여 글개수 가져오기
...
<body class="bg-gray-100" id="page-top">
						...
						<div class="card-body p-0">
							<div class="row">
								<div class="col-lg-12 d-none d-lg-block center-block">
									<h3 class="font-weight-bold text-dark text-left ml-3 my-3"><a href="/notice/noticeList.jsp" style="text-decoration: none; color:inherit;">공지 사항</a></h3>
								</div>
							</div>
						</div>
					</div>
						...
						<div class="table-responsive mt-3">
							<table class="table table-bordered table-hover table-sm mx-auto" style="text-overflow: ellipsis; width: 98%">
								...
								<tbody>
									...
									%>
									<tr onclick="location.href='/notice/noticeView.jsp?num=<%=noticeVO.getNnum()%>&pageNum=<%=pageDTO.getCri().getPageNum()%>'">
										<td><%=noticeVO.getNnum()%></td>
										<td><%=noticeVO.getNsubject()%></td>
										<td><%=noticeVO.getNnick()%></td>
										<td><%=strRegDate%></td>
										<td><%=noticeVO.getNreadcount()%></td>
									</tr>
									<%
									} // for
					...
					<nav aria-label="Page navigation">
						<ul class="pagination justify-content-center my-2">
							...
							<li class="page-item"><a class="page-link"
								href="/notice/noticeList.jsp?pageNum=<%=pageDTO.getStartPage() - 1%>&type=<%=pageDTO.getCri().getType()%>&keyword=<%=pageDTO.getCri().getKeyword()%>#notice"
								aria-label="Previous"
							...
							if (pageDTO.getEndPage() == 0) {
								...
								<li class="page-item <%=(pageDTO.getCri().getPageNum() == i) ? "active" : ""%>"><a class="page-link"
									href="/notice/noticeList.jsp?pageNum=<%=i%>&type=<%=pageDTO.getCri().getType()%>&keyword=<%=pageDTO.getCri().getKeyword()%>#notice"
								><%=i%></a></li>
								<%
								}
							}
							...
							%>
							<li class="page-item"><a class="page-link"
								href="/notice/noticeList.jsp?pageNum=<%=pageDTO.getEndPage() + 1%>&type=<%=pageDTO.getCri().getType()%>&keyword=<%=pageDTO.getCri().getKeyword()%>#notice" aria-label="Next"
						...
						if (id != null) {
							if (id.equals("admin")) {
						%>
						<a href="/notice/noticeWrite.jsp" class="position-absolute" style="margin-top: -45px">
	...
	<script>
		...
		$('#btnSearch').on('click', function() {
			...
			location.href = '/notice/noticeList.jsp?' + query + '#notice';
		});
	</script>
</body>
