...
// 상세보기할 글 한개 가져오기
NoticeVO noticeVO = noticeDAO.getNotice(num);
			...
			<div id="content">
				...
				<div class="container-fluid" style="padding-top: 90px; padding-left: 250px;">
					...
					<div class="card shadow mb-0">
						<input type="hidden" id="mid" name="mid" value="<%=noticeVO.getNmid()%>">

						<div class="text-dark mx-auto mt-4" style="width: 98%">
							<h3 class="font-weight-bold text-left"><%=noticeVO.getNsubject()%></h3>
							<div>
								<ul class="list" style="margin-left: -40px;">
									<li>&nbsp;<b><%=noticeVO.getNnick()%></b>&nbsp;
									</li>
									<li>&nbsp;<b>조회수</b>&nbsp;<%=noticeVO.getNreadcount()%>&nbsp;
									</li>
									<li>&nbsp;<b>작성일</b>&nbsp;<%=sdf.format(noticeVO.getNregDate())%>&nbsp;
									</li>
								</ul>
							</div>
							...
							<div class="btn-toolbar my-2">
								<div class="btn-group mr-2">
									<button type="button" class="btn btn-secondary" onclick="location.href = '/notice/noticeList.jsp'">글목록</button>
									<%
									if (id != null) { // 로그인 확인
										if (id.equals("admin")) {
									%>
									<button type="button" class="btn btn-dark" onclick="location.href='/notice/noticeWrite.jsp'">글쓰기</button>
									<%
									}
									}
									%>
    ...
    // 글삭제 버튼을 클릭하면 호출되는 함수
  	function remove(event) {
    	event.preventDefault(); // a태그 기본동작 막기
  		...
  		if (isRemove == true) {
  			location.href = '/notice/noticeRemove.jsp?num=<%=noticeVO.getNnum()%>&pageNum=<%=pageNum%>';
			}
		}
	</script>
</body>
