...
// humor 테이블에서 전체글 리스트로 가져오기 
List<HumorVO> humorList = humorDAO.getHumors(cri);

// 전체 글개수 가져오기
int totalCount = humorDAO.getCountBySearch(cri); // 검색유형, 검색어가 있으면 적용하여 글개수 가져오기
...
<body class="bg-gray-100" id="page-top">
						...
						<div class="card-body p-0">
							<div class="row">
								<div class="col-lg-12 d-none d-lg-block center-block">
									<h3 class="font-weight-bold text-dark text-left ml-3 my-3"><a href="/humor/humorList.jsp" style="text-decoration: none; color:inherit;">유머 게시판</a></h3>
								</div>
							</div>
						</div>
					</div>
						...
						<div class="table-responsive mt-3">
							<table class="table table-bordered table-hover table-sm mx-auto" style="text-overflow: ellipsis; width: 98%">
								...
								<tbody>
									...
									%>
									<tr onclick="location.href='/humor/humorView.jsp?num=<%=humorVO.getHnum()%>&pageNum=<%=pageDTO.getCri().getPageNum()%>'">
										<td><%=humorVO.getHnum()%></td>
										<td><%=humorVO.getHsubject()%></td>
										<td><%=humorVO.getHnick()%></td>
										<td><%=strRegDate%></td>
										<td><%=humorVO.getHreadcount()%></td>
									</tr>
									<%
									} // for
					...
					<nav aria-label="Page navigation">
						<ul class="pagination justify-content-center my-2">
							...
							<li class="page-item"><a class="page-link"
								href="/humor/humorList.jsp?pageNum=<%=pageDTO.getStartPage() - 1%>&type=<%=pageDTO.getCri().getType()%>&keyword=<%=pageDTO.getCri().getKeyword()%>#humor"
								aria-label="Previous"
							...
							if (pageDTO.getEndPage() == 0) {
								...
								<li class="page-item <%=(pageDTO.getCri().getPageNum() == i) ? "active" : ""%>"><a class="page-link"
									href="/humor/humorList.jsp?pageNum=<%=i%>&type=<%=pageDTO.getCri().getType()%>&keyword=<%=pageDTO.getCri().getKeyword()%>#humor"
								><%=i%></a></li>
								<%
								}
							}
							...
							%>
							<li class="page-item"><a class="page-link"
								href="/humor/humorList.jsp?pageNum=<%=pageDTO.getEndPage() + 1%>&type=<%=pageDTO.getCri().getType()%>&keyword=<%=pageDTO.getCri().getKeyword()%>#humor" aria-label="Next"
						...
						if (id != null) {
						%>
						<a href="/humor/humorWrite.jsp" class="position-absolute" style="margin-top: -45px">
	...
	<script>
		...
		$('#btnSearch').on('click', function() {
			...
			location.href = '/humor/humorList.jsp?' + query + '#humor';
		});
	</script>
</body>
